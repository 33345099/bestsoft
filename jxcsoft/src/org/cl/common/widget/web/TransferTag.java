...
public class TransferTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		Map paramMap = null;
		if (null == transferStr) {
			paramMap = ServletUtils.getParametersStartWith(request, "transfer,filter", true);
		} else {
			paramMap = ServletUtils.getParametersStartWith(request, transferStr, true);
		}
		if (isForm) {
			searchHtml.append("<form id='transferForm'  method='post' style='display:none'>");
		}
